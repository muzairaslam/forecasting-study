...
# Iterate through each variable and perform the first difference transformation, plotting, and ADF test
...
for (i in seq_along(variables)) {
  ...
  # Align fis_yr to the length of diff_variable by removing the first year
  aligned_fis_yr <- data_tsibble$fis_yr[-1]
  
  # Adjust aligned_fis_yr to match the length of diff_variable after NA removal
  aligned_fis_yr <- aligned_fis_yr[1:length(diff_variable)]
  
  # Combine the aligned_fis_yr and diff_variable into a data frame for plotting
  diff_data <- data.frame(fis_yr = aligned_fis_yr, diff_variable = diff_variable)
  
  # Create and save the plot for the first-differenced variable
  p <- ggplot(diff_data, aes(x = fis_yr, y = diff_variable)) +
    geom_line(color = "#900C3F", size = 1.5) +
    labs(title = paste("First-Differenced Time Series Plot of", variable_name), 
         x = "Fiscal Year", 
         y = paste("First Difference of", variable_name)) +
    theme_light()
  
  # Save the plot using a cleaned file name
  ggsave(paste0("visualization/diff_", gsub(" ", "_", gsub("/", "_", tolower(variable_name))), ".png"), plot = p, width = 11.69, height = 8.27, units = "in")
  
  # ADF test and save results
  adf_result <- adf.test(diff_variable)
  sink(paste0("adf_test_results/diff_adf_", gsub(" ", "_", gsub("/", "_", tolower(variable_name))), ".txt"))
  print(adf_result)
  sink()
}
