...
six_vars_lin_log_model <- lm(
  nit_vol_k_ton ~ lg_nit_price_per_ton + 
                  ...
                  log(agric_credit_ratio) +
                  lg_tech_proxy,
                  data = data_tsibble
    )

print(summary(six_vars_lin_log_model))
...
```{r}
six_vars_log_log_model <- lm(
  log(nit_vol_k_ton) ~ lg_nit_price_per_ton + 
                  lg_water_maf +
                  lg_cr_area_mn_hec + 
                  lag_lg_agric_gdp_usd_bn_cy + 
                  log(agric_credit_ratio) +
...
print(summary(six_vars_log_log_model))

# Calculate VIF
vif_values <- vif(six_vars_log_log_model)

# Calculate the ANOVA table for the model
anova_results <- anova(six_vars_log_log_model)
```
...
residuals_df <- data.frame(
  ...
  Predicted_Values = eights_vars_model$fitted.values,
  Historic_Values = data_tsibble$nit_vol_k_ton
)
...
print(summary(eight_vars_lin_log_model))
...
print(summary(eight_vars_log_log_model))

# Calculate VIF
vif_values <- vif(eight_vars_log_log_model)

# Calculate the ANOVA table for the model
anova_results <- anova(eight_vars_log_log_model)
```
